...
## DONE: how to read a plot and the differences between plots. dependent vs independent. x axis vs y. why is time always on the x axis. then put up examples of scatter is individual observations, box shows mdeian and quantiles, and bar plots are summaries. 
...
## DONE: zoom in plot window
## DONE: continuous and categorical variables
## ggplot cheat sheet
## DONE: reminder of == as boolean and == w/ filter
## DONE: double-check when object<- commands vs commands -> object
## DONE: clarify factor vs. variable
...
ex1<- filter(penguins, island == "Biscoe")
ex2<- arrange(ex1, body_mass_g)
...
a<- filter(penguins, island =="Biscoe")
a
b<- select(a,c(1:4, 7:8))
b
c<- mutate(b,bill_length_binary = ifelse(bill_length_mm > 45, "long", "short"))
c
